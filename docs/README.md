# Documentation Index

Use this index as the entry point for repo documentation.

## Core docs

- [`../README.md`](../README.md): public/product-facing project overview.
- [`../ARCHITECTURE.md`](../ARCHITECTURE.md): top-level architecture map.
- [`../CONTRIBUTING.md`](../CONTRIBUTING.md): branch/PR and contributor expectations.
- [`../TESTING.md`](../TESTING.md): build/test/manual validation guidance.

## Interop

- [`drill-package-contract.md`](drill-package-contract.md)
- [`studio-android-compatibility.md`](studio-android-compatibility.md)

## Architecture

- [`architecture/system-overview.md`](architecture/system-overview.md)
- [`architecture/app-modules.md`](architecture/app-modules.md)
- [`architecture/session-lifecycle.md`](architecture/session-lifecycle.md)
- [`architecture/video-pipeline.md`](architecture/video-pipeline.md)
- [`architecture/replay-and-fallback.md`](architecture/replay-and-fallback.md)
- [`architecture/overlay-rendering.md`](architecture/overlay-rendering.md)
- [`architecture/movement-profile-architecture.md`](architecture/movement-profile-architecture.md)

## Features

- [`features/current-user-flows.md`](features/current-user-flows.md)
- [`features/drill-authoring.md`](features/drill-authoring.md)
- [`features/live-coaching.md`](features/live-coaching.md)
- [`features/video-import.md`](features/video-import.md)
- [`features/reference-training.md`](features/reference-training.md)
- [`features/session-history.md`](features/session-history.md)

## Diagrams

- [`diagrams/ui-flow.md`](diagrams/ui-flow.md)
- [`diagrams/architecture-subsystems.md`](diagrams/architecture-subsystems.md)
- [`diagrams/sequence-live-session.md`](diagrams/sequence-live-session.md)
- [`diagrams/sequence-import-analysis.md`](diagrams/sequence-import-analysis.md)
- [`diagrams/sequence-export-finalization.md`](diagrams/sequence-export-finalization.md)
- [`diagrams/class-diagram.md`](diagrams/class-diagram.md)

## ADRs

- [`decisions/adr-001-annotated-export-strategy.md`](decisions/adr-001-annotated-export-strategy.md)
- [`decisions/adr-002-replay-fallback-strategy.md`](decisions/adr-002-replay-fallback-strategy.md)
- [`decisions/adr-003-session-duration-source-of-truth.md`](decisions/adr-003-session-duration-source-of-truth.md)
- [`decisions/adr-004-product-workflow-simplification.md`](decisions/adr-004-product-workflow-simplification.md)

## Maintenance rule

When you change workflows, architecture, naming, navigation, media behavior, or analysis boundaries, update the relevant docs and diagrams in the same PR.
